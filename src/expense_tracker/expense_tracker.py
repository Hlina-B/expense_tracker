...
from expense_tracker.service.helper import buildExpenseObject, updateExpenseObject, any_one_attribute, fliter_by_dm, budget_status
from expense_tracker.service.budget_tracker import check_available_budget
import sys
...
def add(args):
    ...
    budget = check_available_budget(data, expense.createdAt.month)
...
def delete(args):
    ...
    if expense is None:
        print("Deleted successful. The ID provided is not found.")
        return 
    write_file(data)
...
def list_expense(args):
    ...
    print(f'\n{"ID":<5}  |  {"Date":<12}  |  {"Description":<20}  |  {"Amount":<12}  |  {"Category":<12}')
    print("-" * 80)
    for expense in data:
        ...
        print(f'{expense.id:<5}  |  {date:<12}  |  {expense.description:<20}  |  {expense.amount:<12}  |  {category:<12}')
