...
def check_available_budget(expenses: dict[int, Expense], month: int):
    expense_by_month = sum(map(lambda x: x.amount, filter(lambda x: x.createdAt.month == month, expenses.values())), start=0.0)
    budgets = read_budget_file()
    id = build_budget_id(month)
    budget = budgets.get(id)
    if budget is None:
        return
    return budget.amount - expense_by_month